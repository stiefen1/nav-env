...
from nav_env.ships.params import ShipPhysicalParams, DEFAULT_PATH_TO_NEW_JSON
import numpy as np
...
class PhysicsBase(ABC):
    def __init__(self, path_to_params:str=None) -> None:
        self._path_to_params = path_to_params or DEFAULT_PATH_TO_NEW_JSON

    @abstractmethod
    def get_time_derivatives_and_forces(self, states:States3,
                             wind:WindVector,
                             water:WaterVector,
                             control_forces:GeneralizedForces,
                             external_forces:GeneralizedForces
                             ) -> tuple[TimeDerivatives3, GeneralizedForces]:
        pass

    @abstractmethod
    def rotation_matrix(self, psi_rad:float, dim:int=3) -> np.ndarray:
        pass

class ZeroAccelerationPhysics(PhysicsBase):
    ...
    def get_time_derivatives_and_forces(self, states:States3,
        ...
        return TimeDerivatives3(*states.vel, 0., 0., 0.), GeneralizedForces(0., 0., 0., 0., 0., 0.)
...
class ZeroAccelerationSailingShipPhysics(PhysicsBase):
    """
    Heading is enforces to be consistent with x(t), y(t)
    """
    def __init__(self) -> None:
        super().__init__(None)

    def get_time_derivatives_and_forces(self, states, wind, water, control_forces, external_forces):
        x_dot, y_dot = states.vel
        psi_rad = np.arctan2(y_dot, x_dot)
        psi_deg = psi_rad * 180 / np.pi

        error = psi_deg - states.psi_deg # Will make error converge to zero
        # psi = psi + error * dt

        return TimeDerivatives3(x_dot, y_dot, error, 0., 0., 0.), GeneralizedForces(0., 0., 0., 0., 0., 0.)

class ShipPhysics(PhysicsBase):
    def __init__(self, path_to_physical_params:str=None) -> None:
        path_to_physical_params = path_to_physical_params or DEFAULT_PATH_TO_NEW_JSON
        super().__init__(path_to_physical_params)
